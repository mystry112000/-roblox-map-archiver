-- https://github.com/mystry112000/-roblox-map-archiver
...
--[[
	...
	Mystery Decompiling is a debugging suite designed to help the user debug games and find any potential vulnerabilities.
]]
...
return search]==]
	...
	Explorer.Init = function()
		...
		-- Ensure Terrain and CSG objects are visible (special handling for certain object types)
		local workspaceObj = service.Workspace
		if workspaceObj then
			-- Handle Terrain
			local terrain = workspaceObj:FindFirstChildWhichIsA("Terrain") or workspaceObj:FindFirstChild("Terrain")
			if terrain then
				if not nodes[terrain] then
					local workspaceNode = nodes[workspaceObj]
					if workspaceNode then
						local newNode = {
							Obj = terrain,
							Parent = workspaceNode,
						}
						nodes[terrain] = newNode
						workspaceNode[#workspaceNode+1] = newNode
					end
				end
			end

			-- Handle CSG objects (UnionOperation, NegateOperation, PartOperation)
			local descendants = getDescendants(workspaceObj)
			for i = 1,#descendants do
				local obj = descendants[i]
				local className = obj.ClassName
				if className == "UnionOperation" or className == "NegateOperation" or className == "PartOperation" or className == "IntersectOperation" then
					if not nodes[obj] then
						local parObj = ffa(obj,"Instance")
						local par = nodes[parObj]
						if par then
							local newNode = {
								Obj = obj,
			...
			-- Expand Workspace by default so Terrain and CSG objects are visible
			local workspaceNode = nodes[workspaceObj]
			if workspaceNode then
				expanded[workspaceNode] = true
			end
		end

		-- Trigger initial update to refresh the view
		Explorer.ForceUpdate()
	end
...
Main = (function()
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "X" or "Mystery!"
